def index():
    ...
    fields = []
    fields.append(Field('tags', label=db.model.modeltype.label,
                        requires=IS_EMPTY_OR(db.model.modeltype.requires), required=False))

    articles = db(db.article).iterselect(orderby=db.article.name)
...
def update():
    ...
    form = SQLFORM(db.article, request.args(0), upload=URL(
        'default', 'download'), deletable=True, showid=False, _id='articleform')
