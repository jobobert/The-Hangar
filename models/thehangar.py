...
def model_type_icon(model, size):
    ...
    return show_icon(model.modeltype.lower() + '-' + str(size) + '.svg', size, model.modeltype)
...
def model_powerplant_icon(model, size):
    ...
    return show_icon(iconname, size, alt=model.powerplant)
...
def model_control_icon(model, size):
    ...
    return show_icon(iconname, size, alt=model.controltype)
...
def library_type_icon(article, size):
    ...
    return show_icon(iconname, size=32, alt=article.articletype)
